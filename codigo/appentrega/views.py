...
def buscar(request):
    if request.method == "POST":
        ...
        if automoviles.objects.filter(marca_contains= marca):
            ...
            return render (request, "appentrega/buscar.html", {"automovil": automovil})

        elif motos.objects.filter(marca_contains = marca):
            moto = motos.objects.filter(titulos_contains= marca)
            return render ( request, "appentrega/buscar.html", {"moto", moto})
...
def insertar_automoviles(request):
    if request.method == "POST":
          ...
          auto.save()
          return render(request, "appentrega/insertar_automovil.html")
